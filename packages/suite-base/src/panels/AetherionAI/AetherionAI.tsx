...
export function AetherionAI({ context }: AetherionAIProps): React.JSX.Element {
  ...
  const [config, setConfig] = useState<AetherionAIConfig>(() => ({
    ...
    cosmosReasonEndpoint: (context.initialState as Partial<AetherionAIConfig>)?.cosmosReasonEndpoint || COSMOS_REASON_ENDPOINT || "",
    cosmosPredictEndpoint: (context.initialState as Partial<AetherionAIConfig>)?.cosmosPredictEndpoint || COSMOS_PREDICT_ENDPOINT || "",
  }));
  ...
  useEffect(() => {
    const hasEnvKeys = Boolean(COSMOS_REASON_ENDPOINT) && Boolean(COSMOS_PREDICT_ENDPOINT);
      ...
      general: {
        ...
        fields: {
          cosmosReasonEndpoint: {
            label: "Cosmos Reason 2 Endpoint",
            input: "string",
            value: config.cosmosReasonEndpoint,
          },
          cosmosPredictEndpoint: {
            label: "Cosmos Predict2 Endpoint",
            input: "string",
            value: config.cosmosPredictEndpoint,
          },
        },
      },
    };
    ...
    context.updatePanelSettingsEditor({
      actionHandler: (action: SettingsTreeAction) => {
        if (action.action === "update") {
          ...
          if (path[1] === "cosmosReasonEndpoint") {
            setConfig(prev => ({ ...prev, cosmosReasonEndpoint: value as string }));
          } else if (path[1] === "cosmosPredictEndpoint") {
            setConfig(prev => ({ ...prev, cosmosPredictEndpoint: value as string }));
          }
        }
      },
      nodes: settingsTree,
    });
  }, [context, config.cosmosReasonEndpoint, config.cosmosPredictEndpoint]);
      ...
      <Stack fullHeight style={{ overflow: "hidden" }}>
        {config.activeTab === "gapFinder" && (
          <GapFinderTab
            ...
            cosmosReasonEndpoint={config.cosmosReasonEndpoint}
            topicInfo={topicInfo}
          />
        )}
        {config.activeTab === "sim" && (
          <SimTab
            ...
            cosmosEndpoint={config.cosmosPredictEndpoint}

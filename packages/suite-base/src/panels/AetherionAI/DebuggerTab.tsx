...
// Debugger Tab - Analyzes why events happened using NVIDIA Cosmos Reason 2
...
export function DebuggerTab({
  ...
  const askQuestion = useCallback(async () => {
    if (!cosmosReasonEndpoint) {
      setError("Cosmos Reason 2 endpoint not configured. Add it in panel settings.");
      return;
    }
    ...
    const prompt = `You are an autonomous vehicle systems debugger with physical world understanding. Help the user understand what happened in their driving data using physical AI reasoning.
...
## Your Analysis
Based on the ${hasLiveData ? "ACTUAL SENSOR DATA above" : "available topics"}, provide a detailed technical analysis using physical AI reasoning:
...
2. **Physical interpretation**: What does this indicate about vehicle behavior and physical state at this moment?
3. **Anomaly detection**: Are there any unusual values or patterns in the data? Are they physically plausible?
4. **Root cause analysis**: What could explain the observed behavior based on physical world understanding?
...
Be specific and reference the actual data values when available. Provide actionable insights.`;
    ...
    try {
      // Cosmos Reason 2 uses OpenAI-compatible API format via vLLM
      const response = await fetch(`${cosmosReasonEndpoint}/v1/chat/completions`, {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(`Cosmos Reason 2 API error: ${response.status}`);
      }
  ...
  }, [cosmosReasonEndpoint, question, topicInfo, debugHistory, onHistoryUpdate]);
